...
class AuthViewSet(viewsets.ViewSet):
    ...
    def register(self, request):
        ...
        temp_token = generate_temp_token(user_instance)
        return Response({'message': f'Hi {user_instance.username}, thanks for signing up to Greensnow. Check your email for a verification code.', 'temp_token': temp_token}, status=status.HTTP_201_CREATED)
    ...
    def register_manager(self, request):
            ...
            send_code_to_user_email.delay(user_instance.email)
            temp_token = generate_temp_token(user_instance)
            return Response({'message': f'Hi {user_instance.username}, you have been registered as a manager. Check your email for a verification code.','temp_token': temp_token}, status=status.HTTP_201_CREATED)
    ...
    def login(self, request):
        ...
        return Response({'message': 'Login successful. OTP verification required.', 'temp_token': temp_token}, status=status.HTTP_200_OK)
    ...
    def login_manager(self, request):
        ...
        user = serializer.validated_data['user']
        temp_token = generate_temp_token(user)
        return Response({'message': 'Manager login successful.', 'temp_token': temp_token}, status=status.HTTP_200_OK)

        
    @action(detail=False, methods=['post'], url_path='request-otp', authentication_classes=[TempTokenAuthentication])
    @throttle_classes([OTPThrottle])
    def request_otp(self, request):
        serializer = OTPRequestSerializer(data=request.data, context={'request': request, 'purpose': OTP_PURPOSE_ACCOUNT_VERIFICATION})
        serializer.is_valid(raise_exception=True)
        serializer.save()
    ...
    def verify_otp(self, request):
        purpose = request.query_params.get("purpose", OTP_PURPOSE_ACCOUNT_VERIFICATION)
